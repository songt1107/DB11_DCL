 ...
 * 					모든 권한과 책임을 가지는 계정.
 * 					ex) sys(최고관리자), system(sys에서 권한 몇 개가 제외된 관리자)
 * 
 * 
...
CREATE USER swc_sample IDENTIFIED BY 1234;


-- 2. 새 연결(접속방법) 추가
--> SWC_SAMPLE은 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO swc_sample;
...
GRANT CREATE TABLE TO swc_sample;

ALTER USER swc_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO swc_sample;
...
-- kh / swc_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh) swc_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO swc_sample;
...
REVOKE SELECT ON EMPLOYEE FROM swc_sample;
